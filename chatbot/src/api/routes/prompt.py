...
from chatbot.src.models.schemas import ChatRequest, ChatResponse
...
async def ask_gpt(user_question: str, top_answers: list[str]) -> str:
    system_prompt = (
        "Ты — справочный бот. Ниже представлены возможные ответы на вопрос пользователя. "
        "Выбери наилучший ответ и сформулируй на его основе ответ для клиента."
    )
...
async def prompt(request: ChatRequest):
    ...
    try:
        # Шаг 1: запрос к match
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                f"http://localhost:8000/api/requests/match",
        ...
        # Шаг 2: если confident — вернуть напрямую
